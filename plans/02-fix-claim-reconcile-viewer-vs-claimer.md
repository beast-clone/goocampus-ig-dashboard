...
1. In app/(dashboard)/dashboard/preview/my-day/PreviewMyDay.tsx, change line 1211 from `return !!server && server.detail.owner !== meNameRef.current;` to `return !!server && server.detail.owner !== c.detail.owner;` (the `c` param of the `.filter((c) => {...})` at line 1206 is the buffered claim and already has detail.owner = the claimer).
