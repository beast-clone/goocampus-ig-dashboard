...
All Version-2 UI lives under `app/(dashboard)/dashboard/preview/` and renders inside `.preview-scope` (see `app/globals.css`). New V2 UI must stay in that scope.

### Brand tokens (from `.preview-scope` in `app/globals.css`)
...
- **Cards / panels:** white surface, `border border-gray-100`, `rounded-xl` (12px), `rounded-2xl` for large containers. **No drop shadows** — `.preview-scope` strips them; use borders + the gray canvas for separation.
...
- Use Tabler outline icons (`@tabler/icons-react`), the shared `PreviewSelect` for dropdowns, and existing preview components (`PreviewDashboardShell`, `PreviewSidebar`, `Panel`-style cards) rather than reinventing.
- Reference well-built examples already in the repo: the Marketing Hub Master-sheet views + detail modal (`marketing-hub/page.tsx`) and `my-day/PreviewMyDay.tsx`.
...
## Workflow
- Work on branch `feat/dashboard-reskin`. Verify changes live in the user's Chrome before saying they're done. No Netlify production deploy without explicit approval.
