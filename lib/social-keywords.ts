...
type Item = { text: string; tagText?: string; engagement: number; source: "ours" | "competitor"; account: string };
...
async function instagramItems(): Promise<{ items: Item[]; accounts: AccountSummary[] }> {
  ...
  try {
    const own = await igGet<{ data: IgMedia[] }>(`${acc.igUserId}/media`, { fields: "caption,like_count,comments_count", limit: "60", access_token: acc.pageAccessToken });
    for (const m of own.data || []) if (m.caption) items.push({ text: m.caption, engagement: eng(m), source: "ours", account: acc.handle || "goocampus" });
...
// ── YouTube ────────────────────────────────────────────────────────────────
async function channelVideos(handle: string): Promise<{ name: string; subs: number; videos: { text: string; tagText: string; views: number }[] } | null> {
  ...
  const vr = await youtubeGet<{ items?: { snippet?: { title?: string; tags?: string[]; description?: string }; statistics?: { viewCount?: string } }[] }>(`videos?part=snippet,statistics&id=${ids}`);
  return {
...
async function youtubeItems(): Promise<{ items: Item[]; accounts: AccountSummary[] }> {
  ...
  for (const { handle, ours } of all) {
    try {
      ...
      for (const v of r.videos) items.push({ text: v.text, tagText: v.tagText, engagement: v.views, source: ours ? "ours" : "competitor", account: handle });
...
function score(items: Item[], platform: "instagram" | "youtube"): KeywordRow[] {
  type Acc = { kind: KeywordRow["kind"]; accounts: Set<string>; comps: Set<string>; posts: number; eng: number; oursPosts: number; oursEng: number };
  const map = new Map<string, Acc>();
  const bump = (key: string, kind: KeywordRow["kind"], it: Item) => {
    const a = map.get(key) || { kind, accounts: new Set(), comps: new Set(), posts: 0, eng: 0, oursPosts: 0, oursEng: 0 };
    a.accounts.add(it.account); if (it.source === "competitor") a.comps.add(it.account);
    a.posts += 1; a.eng += it.engagement;
    if (it.source === "ours") { a.oursPosts += 1; a.oursEng += it.engagement; }
    map.set(key, a);
  };
...
export function getSocialKeywords(fresh = false): Promise<SocialKeywords> {
  ...
  return fresh ? build() : cached("social-keywords:v1", 24 * 60 * 60_000, build, (d) => d.instagram.length + d.youtube.length > 0);
}
