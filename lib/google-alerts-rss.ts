...
function stripTags(html: string): string {
  // In Atom feeds with type="html", content is entity-encoded (e.g. "&lt;b&gt;text&lt;/b&gt;").
  // Decode entities FIRST so "&lt;b&gt;" becomes "<b>", then strip. Doing it in the
  // opposite order leaves the "<b>" as literal text after decoding.
...
function unwrapGoogleRedirect(link: string): string {
  try {
    // Raw XML hrefs use &amp; instead of & — the URL parser reads that as an
    // "amp;url" param name, not "url", so unwrap silently fails. Decode first.
    const clean = link.replace(/&amp;/g, "&");
    if (!clean.includes("google.com/url")) return clean;
