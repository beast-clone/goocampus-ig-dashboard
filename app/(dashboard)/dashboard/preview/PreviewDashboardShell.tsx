...
import { PreviewShell, type PreviewTab } from "./PreviewShell";

// Themed (Version 2) drop-in replacement for the V1 `DashboardShell`. It exposes
// the EXACT same render-prop API — children receive { accountId, compareAll, range } —
// so any V1 page can be cloned into preview/<tab>/ by swapping the import and
// adding `active`, with zero changes to the page's sections. It renders the themed
// sidebar + a Themed header (account picker + date range + badges), and wraps
// the whole thing in `.preview-scope` so every reused V1 section recolours violet→brand
...
export function PreviewDashboardShell({
  ...
  return (
    <PreviewShell active={active} title={title} hideTopbar>
