...
// PreviewSelect is the themed one the Scheduler already uses.
...
export function TeamView({ rows, allRows, facets, onOpen, loading }: { rows: Row[]; allRows: Row[]; facets?: Facets; onOpen: (id: string) => void; loading: boolean }) {
...
export function PipelineView({ rows, facets, onOpen, loading }: { rows: Row[]; facets?: Facets; onOpen: (id: string) => void; loading: boolean }) {
            ...
            {attention.oldest.map(({ r, age }) => {
              ...
              return (
                <button key={r.id} onClick={() => onOpen(r.id)} className="w-full text-left grid grid-cols-[minmax(0,2.5fr)_1.6fr_1.5fr_1.5fr] items-center gap-x-4 px-2.5 py-2.5 rounded-lg hover:bg-gray-50 border-b border-gray-50 last:border-0">
                  ...
                  <span className="justify-self-start inline-flex items-center gap-2 min-w-0 whitespace-nowrap">
                    <span className={`text-[11px] font-semibold rounded-md px-2 py-0.5 ${age >= 7 ? "bg-rose-50 text-rose-700" : "bg-amber-50 text-amber-700"}`}>{age} {age === 1 ? "day" : "days"} old</span>
                    <span className="text-[11px] text-gray-400">{fmtDateShort(r.lastModified || r.createdDate)}</span>
                  </span>
                  ...
                  <span className="justify-self-start inline-flex items-center gap-1.5 text-xs text-gray-600 whitespace-nowrap min-w-0">
                    ...
                    <span className="truncate">{r.owner || "Unassigned"}</span>
                  </span>
                </button>
              );
            })}
...
// Status → pill palette. Mirrors the dashboard publishing calendar's approach: soft-tinted
...
// Mirrors the Publishing Calendar (calendar/PreviewCalendar.tsx / theme reference) so the two
...
export function CalendarView({ rows, facets, onOpen, onSaved, loading }: { rows: Row[]; facets?: Facets; onOpen: (id: string) => void; onSaved: () => void; loading: boolean }) {
  ...
  const brandCounts = useMemo(() => {
    ...
    for (const r of channelRows) m.set(r.sbu, (m.get(r.sbu) || 0) + 1);
    return m;
  }, [channelRows]);
      ...
      <div className="space-y-2">
        {sorts.map((s, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0"><PreviewSelect value={s.field} onChange={(v) => update(i, { field: v })} options={fields.map((f) => ({ value: f.key, label: f.label }))} /></div>
            <div className="w-[132px] flex-shrink-0"><PreviewSelect value={s.dir} onChange={(v) => update(i, { dir: v as "asc" | "desc" })} options={[{ value: "asc", label: "First → last" }, { value: "desc", label: "Last → first" }]} /></div>
...
function AddColumnModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-[60] flex items-center justify-center p-6 preview-scope" onClick={onClose}>
...
// Themed card: white surface, hairline border, rounded-xl, labelled header (optional
  ...
  const editBox = (rows: number, placeholder: string) => (
    <div className="space-y-2">
      <textarea value={draft} onChange={(e) => setDraft(e.target.value)} rows={rows} autoFocus placeholder={placeholder}
        className="w-full border border-gray-200 rounded-lg p-2.5 text-[14px] leading-relaxed font-normal focus:outline-none focus:ring-1 focus:ring-brand" />
