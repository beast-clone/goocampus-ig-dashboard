...
import {
  ...
  IconArrowUpRight, IconArrowDownRight, IconBrandInstagram,
...
export function HopeOverview() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/insights?accountId=goocampus&from=${from}&to=${to}`)
      .then((r) => r.json())
      ...
      <main style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ padding: 28, display: "flex", flexDirection: "column", gap: 22, maxWidth: 1320, margin: "0 auto" }}>
          ...
          <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 18 }}>
            {loading && !data
              ? [0, 1, 2, 3, 4].map((i) => <div key={i} style={{ height: 108, background: C.card, borderRadius: 14, boxShadow: SHADOW }} />)
              : stats.map((s) => <StatCard key={s.label} {...s} />)}
          </section>

          {/* Chart */}
          {data && (
            <section style={{ background: C.card, borderRadius: 16, boxShadow: SHADOW, padding: "22px 24px" }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12, marginBottom: 8 }}>
                <div>
                  <div style={{ fontSize: 24, fontWeight: 700, color: C.heading, letterSpacing: "-0.4px" }}>{fmt(t!.reach)}</div>
                  <div style={{ fontSize: 13, color: C.muted }}>Reach this period</div>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
                  <Legend color={C.primary} label="Reach" />
                  <Legend color={C.teal} label="Engagement" />
          ...
          <div style={{ fontSize: 11.5, color: C.muted, textAlign: "center", paddingBottom: 8 }}>
            Hope UI style · proof page · real @goocampus data · <span style={{ color: C.primary, fontWeight: 600 }}>feat/hope-ui-reskin</span>
          </div>
        </div>
      </main>
    </div>
...
function NavItem({ icon: Icon, label, active }: { icon: typeof IconLayoutGrid; label: string; active?: boolean }) {
  return (
    <div style={{
      display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", borderRadius: 10, marginBottom: 2, cursor: "pointer",
      background: active ? C.primary : "transparent",
      color: active ? "#fff" : C.muted,
      fontWeight: active ? 600 : 500, fontSize: 14,
      boxShadow: active ? "0 8px 18px rgba(58,87,232,0.30)" : "none",
    }}>
      <span style={{ width: 30, height: 30, borderRadius: 8, display: "grid", placeItems: "center", background: active ? "rgba(255,255,255,0.18)" : C.chip, color: active ? "#fff" : C.muted, flexShrink: 0 }}>
...
function StatCard({ label, value, delta, flat }: { label: string; value: string; delta: number; flat?: boolean }) {
  const up = delta >= 0;
  const pct = Math.min(100, Math.abs(delta) * 4 + 12); // arc fill (visual)
  const R = 24, CIRC = 2 * Math.PI * R;
  const col = flat ? C.primary : up ? C.success : C.danger;
  return (
    <div style={{ background: C.card, borderRadius: 14, boxShadow: SHADOW, padding: "18px 18px", display: "flex", alignItems: "center", gap: 16 }}>
      <div style={{ position: "relative", width: 58, height: 58, flexShrink: 0 }}>
        <svg width="58" height="58" viewBox="0 0 58 58">
          ...
          <circle cx="29" cy="29" r={R} fill="none" stroke={col} strokeWidth="5" strokeLinecap="round"
            strokeDasharray={CIRC} strokeDashoffset={CIRC * (1 - pct / 100)} transform="rotate(-90 29 29)" />
        </svg>
        <span style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", color: col }}>
          {flat ? <IconChartLine size={18} /> : up ? <IconArrowUpRight size={19} /> : <IconArrowDownRight size={19} />}
        </span>
      </div>
      <div style={{ minWidth: 0 }}>
        <div style={{ fontSize: 12, color: C.muted, fontWeight: 500 }}>{label}</div>
        <div style={{ fontSize: 23, fontWeight: 700, color: C.heading, letterSpacing: "-0.5px", lineHeight: 1.15 }}>{value}</div>
...
function AreaChart({ series }: { series: Insights["series"] }) {
  ...
  const path = useMemo(() => {
    if (pts.length < 2) return { area: "", line: "", eng: "", maxR: 1 };
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", display: "block" }} preserveAspectRatio="none">
      ...
      {[0.25, 0.5, 0.75, 1].map((g) => (
        <line key={g} x1={PL} x2={W - PR} y1={PT + g * (H - PT - PB)} y2={PT + g * (H - PT - PB)} stroke={C.line} strokeWidth="1" />
      ))}
