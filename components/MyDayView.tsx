...
export function MyDayView({ range, lockedPersonKey }: { range: { from: string; to: string }; lockedPersonKey?: string }) {
  ...
  return (
    <div className="space-y-5">
      {/* Person switcher (admin) — hidden when locked to the signed-in member */}
      <div className="bg-white border border-gray-100 rounded-lg p-2 flex items-center gap-2">
        {lockedPersonKey ? (
          <div className="text-sm text-gray-500 px-3">Your queue — live from the team&apos;s content board</div>
        ) : (
          <>
